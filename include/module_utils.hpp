...
torch::nn::Conv2d convNxN(int64_t in_channels, int64_t out_channels, int64_t stride = 1) {
    return torch::nn::Conv2d(torch::nn::Conv2dOptions(in_channels, out_channels, N)
                                 .stride(stride)
                                 .padding(1)
                                 .bias(false));
}
